...
function build() {
  ...
  c.push(...BUL([
    ...
    'Hyperparameters are selected on a validation block of only ten origins, which cannot separate four candidates reliably; on this configuration that selection is worse than using library defaults. We report the reversal rather than widen the block until the sign turns.',
    ...
    'The three leading methods are statistically indistinguishable, so the order within that group in Table 6 carries no weight. Pairwise tests against the leading method were not corrected for multiple comparisons, which would widen the tie group rather than narrow it.',
    'Only one horizon is studied, and the panel comes from one jurisdiction and one reporting framework. A managed float and a shallow onshore market both raise the weight of counterparty composition, so the results should not be carried to a deep free-floating market, or to a longer horizon, without re-testing.',
    ...
    `ARIMA is not reproducible run to run. Re-running the identical code and data on the same machine, with the only change being that each process was pinned to a single compute thread, moved ${T.arima_repro.sel_berbeda.ARIMA} of the ${T.arima_repro.n_per_model} ARIMA cells; its pooled mean shifted from ${n(T.arima_repro.mean_lama, 4)} to ${n(T.arima_repro.mean_baru, 4)} and its test against the leading method from p = ${pv(T.arima_repro.p_lama)} to p = ${pv(T.arima_repro.p_baru)}. The order-selection search is sensitive to floating-point summation order. No other method moved. Conclusions are unaffected, but ARIMA figures at the fourth decimal should not be treated as exact.`,
    `The numbers are tied to one library stack, recorded with the results: Python ${T.versi.python}, NumPy ${T.versi.numpy}, pandas ${T.versi.pandas}, scikit-learn ${T.versi.sklearn}, LightGBM ${T.versi.lightgbm}, XGBoost ${T.versi.xgboost}, statsmodels ${T.versi.statsmodels}. Re-running the same code under a different stack reproduced the tree-ensemble results only in part; gradient-boosting output in particular differed. Replication should pin these versions.`,
